...
import { CalculationMode, SubscriptionStatus } from '@/core/types/enums'
...
export class SubscriptionDetails extends ValueObject<SubscriptionDetailsProps> {
    ...
    get billingSuccessRate() {
        ...
        return (this.successfulBillings / this.totalBillings) * 100
    }
    ...
    getDaysUntilNextBilling(): number {
        const now = new Date()
        return Math.ceil((this.nextBillingDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
    }
