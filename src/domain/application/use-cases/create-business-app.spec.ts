...
describe("Create Business App", () => {
  ...
  it("shou0ld be able to create a business app", async () => {
    ...
    expect(inMemoryBusinessAppRepository.items[0]).toEqual(
      result.value?.businessApp
    );
  });
  ...
  it("should persist business app data correctly", async () => {
    ...
    expect(createdBusinessApp?.businessId).toEqual(
      new UniqueEntityID("123e4567-e89b-12d3-a456-426614174000")
    );
    expect(createdBusinessApp?.appId).toEqual(
      new UniqueEntityID("550e8400-e29b-41d4-a716")
    );
