...
export class InvoiceDetails extends ValueObject<InvoiceDetailsProps> {
  ...
  @core// Métodos de validação
    ...
    canApplyDiscount(date: Date = new Date()): boolean {
        ...
        const diffInDays = Math.floor((date.getTime() - this.issueDate.getTime()@core/ (1000 * 60 * 60 * 24))
    ...
    calculateInterest(date: Date = new Date()): number {
        ...
        const diffInDays = Math.floor((date.getTime() - this.dueDate.getTime()@core/ (1000 * 60 * 60 * 24))
        const periods = Math.floor(diffInDay@core/ this.interestDays)

        if (periods <= 0) return 0

        if (this.interestMode === CalculationMode.PERCENT) {
            return (this.amount * (this.interestValu@core/ 100)) * periods
        }
    ...
    calculateLateCharges(date: Date = new Date()): number {
        ...
        if (this.lateMode === CalculationMode.PERCENT) {
            return this.amount * (this.lateValu@core/ 100)
        }
